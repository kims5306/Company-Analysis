...
//    sjDiv는 문자열 또는 배열 — 예: 손익은 ['IS','CIS'](일부 기업은 IS 없이 포괄손익 CIS만 제출).
function pickField(list, sjDiv, accountNm, missSink) {
  const divs = Array.isArray(sjDiv) ? sjDiv : [sjDiv];
  const inDiv = (i) => divs.includes(i.sj_div);
  const ids = (ACCT_IDS[accountNm] || []);
  for (const id of ids) {
    const hit = (list || []).find(i => inDiv(i) && (i.account_id || '') === id);
  ...
  for (const target of candidates) {
    const hit = (list || []).find(i => inDiv(i) && normNm(i.account_nm) === target);
    if (hit) return { amt: toNum(hit.thstrm_amount), add: toNum(hit.thstrm_add_amount) };
  }
  ...
  for (let y = startY; y <= curY; y++) {
    for (const [q, rc] of REPRT) {
      jobs.push((async () => {
        ...
        // 🔴 손익은 IS 또는 CIS(포괄손익계산서) — 일부 기업(일신방직 등)은 별도 IS 없이 CIS만 제출.
        const IS_DIVS = ['IS', 'CIS'];
        const is = {}; IS_ACCTS.forEach(a => { is[a] = pickField(d.list, IS_DIVS, a, missSink); });   // {amt,add}
        // 🔴 순이익 = account_id 'ifrs-full_ProfitLoss'(K-IFRS 표준, 계정명 변형에 불변) 우선.
        //    폴백: 계정명 자동탐색(당기/분기/반기순이익 ± (손실), 법인세·주당·계속영업·지배/비지배 제외).
        const inISDiv = (it) => IS_DIVS.includes(it.sj_div);
        let niHit = d.list.find(it => inISDiv(it) && (it.account_id || '') === 'ifrs-full_ProfitLoss');
